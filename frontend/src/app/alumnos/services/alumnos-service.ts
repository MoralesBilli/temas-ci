...
export class AlumnosService {
  readonly alumnos = resource({
    async loader() {
      await sleep(500)

      return alumnoSchema.array().parse([
        {
  ...
  readonly alumnoSeleccionadoDetalle = resource({
    params: () => ({ alumno: this.alumnoSeleccionado() }),
    async loader({ params: { alumno } }) {
